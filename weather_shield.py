...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("weather_shield.log"), logging.StreamHandler()],
)
...
class WeatherMonitor:
    ...
    def __init__(self, config_path: str = "config.json"):
        ...
        if not self.api_key or not self.latitude or not self.longitude:
            self.logger.warning(
                "Missing required config: api_key, latitude, longitude - Weather monitoring disabled until configured"
            )
            self.disabled = True
        else:
            self.disabled = False
    ...
    def _initialize_computers(self):
        ...
        for comp in computers_config:
            ...
            self.logger.info(
                f"Initialized computer: {self.computers[comp_id]['name']} ({comp_id})"
            )
    ...
    def _send_ntfy_notification(
        ...
        try:
            ...
            headers = {"Title": title, "Priority": priority, "Tags": "weather-shield"}
    ...
    def get_forecast_data(self) -> Optional[Dict[str, Any]]:
    ...
    def check_current_weather(self) -> bool:
        ...
        is_bad = (
            weather_code in self.BAD_WEATHER_CODES
            or weather_desc in self.BAD_WEATHER_CONDITIONS
        )
    ...
    def check_forecast_weather(self) -> bool:
        ...
        if not has_bad_weather:
            self.logger.info(
                f"No bad weather forecasted in next {self.forecast_minutes} minutes"
            )
    ...
    def boot_computer(self, computer_id: Optional[str] = None):
        ...
        for comp_id in target_computers:
            ...
            self.logger.info(
                f"Weather conditions improved for {comp['name']}. Computer can boot."
            )
            self._send_ntfy_notification(
                "✅ Weather Improved",
            ...
            try:
                ...
                if system == "win32":
                    self.logger.info(
                        f"Boot signal for {comp['name']} on Windows (requires Wake-on-LAN)"
                    )
                elif system in ["linux", "linux2"]:
                    self.logger.info(
                        f"Boot signal for {comp['name']} on Linux (requires Wake-on-LAN from another machine)"
                    )
                elif system == "darwin":
                    self.logger.info(
                        f"Boot signal for {comp['name']} on macOS (requires Wake-on-LAN)"
                    )
    ...
    def run(self):
        ...
        if self.disabled:
            self.logger.info(
                "Weather monitoring disabled - waiting for configuration..."
            )
            ...
            while self.running:
                ...
                try:
                    ...
                    if self.api_key and self.latitude and self.longitude:
                        self.disabled = False
                        self.logger.info(
                            "Configuration loaded! Weather monitoring enabled."
                        )
                        break
                except Exception as e:
...
class DashboardApp:
    ...
    def __init__(self, config_path: str = "config.json"):
        ...
        self.running = True
        self.monitor_ref: Optional[WeatherMonitor] = None
    ...
    def _setup_routes(self):
        ...
        def add_computer():
            ...
            try:
                ...
                if not data.get("name"):
                    return (
                        self.jsonify(
                            {"success": False, "message": "Computer name is required"}
                        ),
                        400,
                    )
        ...
        def update_computer(computer_id):
            ...
            try:
                ...
                computers = config.get("computers", [])
                computer = next(
                    (c for c in computers if c.get("id") == computer_id), None
                )

                if not computer:
                    return (
                        self.jsonify(
                            {"success": False, "message": "Computer not found"}
                        ),
                        404,
                    )
                ...
                self.logger.info(
                    f"Updated computer: {computer['name']} ({computer_id})"
                )
        ...
        def delete_computer(computer_id):
            ...
            try:
                ...
                config["computers"] = [
                    c for c in computers if c.get("id") != computer_id
                ]
                ...
                return (
                    self.jsonify(
                        {"success": True, "message": "Computer deleted successfully"}
                    ),
                    200,
                )
            ...
            try:
                ...
                if hasattr(self, "monitor_ref") and self.monitor_ref:
                    if computer_id in self.monitor_ref.computers:
                        ...
                        self.logger.info(
                            f"Manual power control: {comp['name']} - turning {action}"
                        )
                    ...
                    return (
                        self.jsonify(
                            {"success": False, "message": "Monitor not available"}
                        ),
                        500,
                    )
            except Exception as e:
                ...
                return (
                    self.jsonify(
                        {"success": False, "message": f"Error toggling power: {str(e)}"}
                    ),
                    500,
                )
    ...
    def _get_forecast_data(self) -> Dict:
        ...
        try:
            ...
            if (
                ...
                return {"error": "Configuration required", "forecasts": []}
    ...
    def start(self, dashboard_port: int = 5000):
        ...
        try:
            ...
            self.dashboard_thread = threading.Thread(
                target=self.dashboard.run, args=("0.0.0.0", dashboard_port), daemon=True
            )
